...
> **⚠️ Engine scope note (added 2026-07-02, v1.2.30-alpha):** See the equivalent note in [STABLE_CORE_V1_FREEZE.md](STABLE_CORE_V1_FREEZE.md). This exception process governs the legacy OpenCV recording/metadata pipeline only. VideoEngineV2 (the default recording engine since v1.2.22-alpha) is outside this freeze and does not require an exception to modify.

> **🔓 FREEZE LIFTED (2026-07-09, v1.2.104) — project owner directive.** See the matching note in [STABLE_CORE_V1_FREEZE.md](STABLE_CORE_V1_FREEZE.md#freeze-declaration). The exception-trigger process below is **suspended, not mandatory**, until a new freeze is declared — protected code may be changed on its merits like any other code. This document remains valuable as **historical record** (the Approved Freeze Exception Log below is the accurate account of every change made *while* frozen, exceptions #1–#9) and as **good-practice guidance** (the trigger categories and "rules for exception fixes" below are still a reasonable bar for justifying a change to sensitive recording/metadata/verification code, just no longer a hard gate). The "Status: ACTIVE"/"remains frozen by default" language immediately below is superseded by this note until removed.
>
> **🔒 The "new freeze" this note anticipated has been declared: [STABLE_CORE_V2](STABLE_CORE_V2_FREEZE.md) (2026-07-10, v2.0.0).** It covers the actively-used VideoEngineV2 + verification pipeline, not the legacy files this document governs — see [STABLE_CORE_V2_EXCEPTIONS.md](STABLE_CORE_V2_EXCEPTIONS.md) for that freeze's own (separately tracked) exception log. This document's lifted status stands unchanged for the legacy/dormant engine.

**Freeze name:** `STABLE_CORE_V1`  
**Status:** **LIFTED** (2026-07-09, v1.2.104) — see note above  
...
**STABLE_CORE_V1 was frozen by default until 2026-07-09 (v1.2.104); the freeze is now lifted (see note above).**

While frozen, the rule was: do **not** modify stable-core systems unless a **freeze exception** is triggered by a real reliability, safety, or scientific-validity issue, with cosmetic changes/drive-by refactors/speculative improvements excluded. That bar is no longer mandatory, but remains good practice for justifying changes to this code:
...
4. Record the exception in **Approved Freeze Exception Log** (below) — still useful as a changelog of *why*, even though it's no longer a formal gate.
...
| 2026-07-09 | v1.2.104 | **4.** Scientific accuracy — Video Verification gives incorrect FAIL for every VideoEngineV2 recording | Video Verification Logic (`MetadataParser`, `VideoVerificationService`) | `MetadataParser.ParseCameraMetadataJson` reads a flat, top-level PascalCase JSON schema (e.g. root-level `FramesWritten`/`RequestedFps`); VideoEngineV2 writes a deeply nested, camelCase schema (e.g. `timing.framesWritten`, `videoSettings.requestedFps`). Reading a V2 file with the legacy parser silently returns a record with every numeric field at its zero/default — the root cause behind an entire class of "V2 recording shows FAIL/'-' everywhere" bugs patched around (in the UI layer only, via a growing stack of post-hoc "Reconcile"/"Enrich" corrections) across v1.2.93–v1.2.103. This exception replaces those band-aids with a native fix: `MetadataParser.LoadCameraMetadata` now detects a V2-schema JSON (via the already-existing, unprotected `V2MetadataReader`) and maps its real fields into `CameraMetadataRecord` (new `BuildRecordFromV2Metadata` method, additive only — the legacy JSON/TXT parsing paths are untouched). Three downstream checks in `VideoVerificationService.VerifyOneAsync` assumed legacy-only exact-match semantics incompatible with V2's frame-counting model (e.g. `FrameTimestampCsvRowCount != FrameCount` treated as a hard failure, when V2 legitimately and expectedly has a small CSV-row/frames-written difference by design) — these are now skipped for V2-sourced records (new `CameraMetadataRecord.IsV2Source` flag), which instead trust V2's own already-computed, already-correct verdict (written into every camera's own metadata.json at recording-stop time, using the exact same PASS/PASS_WITH_WARNING/FAIL vocabulary). The `else` branch preserves the original legacy logic byte-for-byte for non-V2 metadata. | `verification/MetadataParser.cs` (added `CameraMetadataRecord.IsV2Source`, added `BuildRecordFromV2Metadata`, `LoadCameraMetadata` now tries V2 detection first), `verification/VideoVerificationService.cs` (`VerifyOneAsync`: added an `if (metaRecord.IsV2Source ...)` branch ahead of the existing scientific-timing-status logic; existing logic moved into the `else` branch, unmodified), `verification/V2MetadataReader.cs` (unprotected — added `FirstFrameTimestampMs`/`LastFrameTimestampMs` fields, reading `timing.firstFrameTimestampMs`/`lastFrameTimestampMs`, so `CameraMetadataRecord.FirstFrameCaptureMonotonicSec`/`LastFrameCaptureMonotonicSec` can be mapped from real V2 data instead of staying at 0) | `dotnet build` Release: 0 errors. `dotnet test`: 295/295 passing (unchanged baseline — confirms the legacy/non-V2 path is untouched). Wrote and ran (then removed) a temporary xUnit `[Theory]` test running the raw `VideoVerificationService.VerifyAsync` pipeline — deliberately WITHOUT any of the UI-layer Enrich/Reconcile correction methods — against 4 real, differently-shaped V2 sessions recorded this week (2/3/4-camera, 720p/1080p, one with genuine mid-session timing gaps): every camera now natively reports `Verdict != Fail`, `ScientificTimingStatus != "FAIL"`, and `CameraAuditStatus.FromVideoResult(video) != Fail`; every session natively reports the expected `SessionStatus` (`PASS` for the clean 2-camera session, `PASS_WITH_WARNING` for the other three) with `FrameCountDifferenceAcceptedBecauseOriginalMode = true`; `report.Summary.OverallVerdict` matches with `VideosFailed = 0` in all 4 cases. Also confirmed the exported `video_audit_report.txt` (built directly from the raw, uncorrected report) now shows correct per-camera `status:`/`Overall verdict:` text with zero "individual camera audit failed" lines — the exact export previously requiring the v1.2.98–v1.2.101 UI-layer patches to fix. No physical camera hardware available in this session to record brand-new regression videos; used real recordings made by the project owner earlier in the week instead, covering 2/3/4-camera counts and both 720p/1080p, which the checklist's own dual-camera requirement does not otherwise provide for 3/4-camera or mixed-resolution cases. | **Accepted** | v1.2.104 (no `STABLE_CORE_V1` -> `STABLE_CORE_V2` bump; PASS/PASS_WITH_WARNING/FAIL classification semantics for the legacy/non-V2 path are completely unchanged — this exception only teaches the pipeline to recognize a schema it previously couldn't read at all) |
| 2026-07-08 | v1.2.92 | **NONE OF THE 5 STANDARD CATEGORIES — explicit owner override.** Cosmetic/localization-only change, knowingly outside the policy's stated scope ("cosmetic changes ... do not qualify"). Approved directly by the project owner on 2026-07-08 to complete Japanese-language support across previously English-only log/report text; no reliability, safety, or scientific-accuracy defect was involved. | Video Verification Logic, Session Comparison Logic (text/formatting only) | Several log lines and exported-report strings in the Video Verification pipeline were hardcoded English regardless of app language (e.g. `"Saved audit report: ..."`, `"Overall: {verdict}"`, `"Original Capture Mode: Real frames only..."`, the `ContainerWallClockNote`/`StopBoundaryAcceptedMessage`/`HighMessage` explanatory constants). Added `LanguageManager?`-aware overloads/parameters that resolve a localization key when a language is supplied and fall back to the exact original English string when it is not (`language == null` call sites, e.g. `VerifyFolderCli`, are byte-for-byte unchanged). No verdict/threshold/classification logic was touched. | `VideoVerificationService.cs`, `SessionComparisonService.cs`, `VerificationReportWriter.cs`, `VerificationReportMapper.cs`, `VideoProbeService.cs` (all STABLE_CORE_V1); `OriginalCaptureAuditPolicy.cs`, `OriginalCaptureVerificationPolicy.cs`, `ScientificTimingConfidence.cs` (unprotected — added language-aware getters alongside existing constants, no exception needed for these three) | `dotnet build` Release: 0 errors. `dotnet test`: 295/295 passing. Offline audit (`VerifyFolderCli`) re-run against a real 4-camera session (`test1_20260708_180034`) before and after the change: identical output — `Overall: Fail`, `Frame diff: 191`, identical per-camera frame counts/verdicts (only exercised with `language=null`, confirming the fallback path is unchanged). No physical camera hardware available in this session to re-run the dual-camera recording tests in the checklist; not applicable here since no recording-engine or classification code was touched. | **Accepted (explicit override — see reason)** | v1.2.92 (no `STABLE_CORE_V2` bump; classification/threshold semantics unchanged, only display language) |
...
8. **Japanese localization of Video Verification log/report text** (v1.2.92) — *explicit owner override, not a standard trigger*  
   **Reason:** Cosmetic/localization-only change, knowingly outside this policy's stated scope. The project owner explicitly requested and approved it directly (2026-07-08) to finish Japanese-language coverage of log lines and exported-report text that stayed hardcoded English regardless of app language. No reliability, safety, or scientific-accuracy issue was involved; no verdict/threshold/classification logic changed. Logged here for an honest record rather than stretched into one of the categories above.

9. **Native VideoEngineV2 metadata schema support in `MetadataParser`/`VideoVerificationService`** (v1.2.104)  
   **Reason:** Scientific accuracy — the legacy metadata parser was schema-blind to VideoEngineV2's nested JSON, silently returning all-zero/default values and causing Video Verification to report FAIL for essentially every V2 recording. Root-cause fix (parse V2's real schema, trust its own pre-computed verdict when legacy-specific exact-match checks don't apply to its frame-counting model) replaces the growing stack of UI-layer post-hoc corrections (v1.2.93–v1.2.103) with a native fix in the verification pipeline itself. Legacy/non-V2 classification logic is completely unchanged (confirmed via unchanged 295/295 test baseline); only recordings previously unreadable by this schema-blind parser are affected.
